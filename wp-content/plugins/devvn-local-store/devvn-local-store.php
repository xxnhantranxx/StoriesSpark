...
if ( !class_exists( 'DevVN_Local_Store_Class' ) ) {
    ...
    class DevVN_Local_Store_Class
    {
        ...
        public function admin_footer_text( $text ) {
            $current_screen = get_current_screen();
            if ( isset( $current_screen->post_type ) && $current_screen->post_type == 'local-store' ) {
                $text = sprintf( __( 'Developed by %1$sLê Văn Toản%2$s.', 'devvn-local-store' ), '<a href="https://levantoan.com" target="_blank"><strong>', '</strong></a>' ); //phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
            }
            return $text;
        }
        ...
        function dvls_admin_notice() {
            ...
            if ( isset( $current_screen->post_type ) && $current_screen->post_type == 'local-store' && !$dvls_settings['maps_api'] ) {
                echo '<div class="error"><p><strong>' . sprintf( __('You haven\'t set Google Maps Api yet. Please go to the <a href="%s">plugin admin page</a> to set.', 'devvn-local-store' ), admin_url( 'edit.php?post_type=local-store&page=devvnls_settings' ) ) . '</strong></p></div>'; //phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
            }
        }
